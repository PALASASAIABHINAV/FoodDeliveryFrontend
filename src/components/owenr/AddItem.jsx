...
const AddItem = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/"); // keep same redirect as your code
    } catch (err) {
        ...
        <header className="mb-6 flex items-start justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mt-1 flex items-center gap-2">
              Add new item
              <span className="inline-flex items-center justify-center rounded-full bg-emerald-50 text-emerald-600 text-[10px] font-semibold px-2 py-0.5 border border-emerald-100">
                Live on ZentroEat
              </span>
            </h1>
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              Create a delicious dish card with image, price and availability.
            </p>
          </div>
        </header>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between gap-3 pt-2">
              ...
              <button
                type="submit"
                disabled={loading}
                className="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-full bg-emerald-500 text-xs font-semibold text-white shadow-md hover:bg-emerald-600 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                <CheckCircle2 className="w-4 h-4" />
                {loading ? "Adding item…" : "Save & publish item"}
              </button>
            </div>
          </form>
        </div>
